...
dir.create("results/D1_all/plots/16_maaslin2",  recursive = TRUE, showWarnings = FALSE)
dir.create("results/D1_all/tables/16_maaslin2", recursive = TRUE, showWarnings = FALSE)

# ============================================================
# Helper: agglomerate to taxonomic rank, clean names,
#         return samples x features matrix for MaAsLin2
# ============================================================
...
# -- Heatmap: significant taxa x terms, fill = coefficient ----
...
make_heatmap <- function(res, title, subtitle, rank) {
  ...
  # Order rows by Species coefficient
  sp_order <- df %>%
...
# -- Volcano: coef vs -log10(q), faceted by term ---------------
...
make_volcano <- function(res, title, subtitle) {
  ...
  # Label top 10 per term by q-value
  top_labels <- df %>%
    filter(qval < SIG_Q) %>%
    group_by(term) %>%
...
# -- Scatter: Species coef vs sample_type coef, one point per taxon --
...
for (rank in tax_levels) {
  ...
  otu_mat <- prepare_level(PS_merged_sanger, rank)
  ...
  for (thr_name in names(threshold_configs)) {
    ...
    out_dir  <- file.path("results/D1_all/tables/16_maaslin2", tag)
    plot_dir <- file.path("results/D1_all/plots/16_maaslin2",  tag)
    ...
    fig_title    <- paste0("Differential abundance \u2014 ", rank, " level")
...
cat("\n=== All MaAsLin2 D1 analyses complete ===\n")
cat("Results: results/D1_all/tables/16_maaslin2/\n")
cat("Figures: results/D1_all/plots/16_maaslin2/\n")
cat("  Subfolders: {level}_filtered | {level}_unfiltered\n")
