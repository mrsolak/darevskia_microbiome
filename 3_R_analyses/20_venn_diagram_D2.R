...
OTU <- as.data.frame(as.matrix(otu_table(PS_D2)))
if (!taxa_are_rows(PS_D2)) OTU <- t(OTU)

dir.create("results/D2_paired/plots/20_venn_diagram", recursive = TRUE, showWarnings = FALSE)
dir.create("results/D2_paired/tables/20_venn_diagram", recursive = TRUE, showWarnings = FALSE)
...
cat("=== Sample counts per group (D2) ===\n")
...
p_combined <- wrap_plots(panels, nrow = 1) +
  plot_annotation(
    title = "ASV overlap across species and intestine types — D2 paired",
    theme = theme(plot.title = element_text(face = "bold", size = 13, hjust = 0.5))
  )

ggsave("results/D2_paired/plots/20_venn_diagram/venn_4way_thresholds.png",
       p_combined, width = 18, height = 7, dpi = 300, bg = "white")

p_single <- make_venn(all_sets[[1]], "No threshold (\u2265 1 sample)")
ggsave("results/D2_paired/plots/20_venn_diagram/venn_4way.png",
...
for (k in seq_along(thresholds)) {
  ...
  write.csv(summary_df,
            sprintf("results/D2_paired/tables/20_venn_diagram/venn_summary_thr%s.csv", thr_tag),
            row.names = FALSE)
  write.csv(as.data.frame(overlap_mat),
            sprintf("results/D2_paired/tables/20_venn_diagram/venn_overlaps_thr%s.csv", thr_tag),
            row.names = TRUE)
}

cat("\nOutputs saved to:\n")
cat("  results/D2_paired/plots/20_venn_diagram/\n")
cat("  results/D2_paired/tables/20_venn_diagram/\n")
