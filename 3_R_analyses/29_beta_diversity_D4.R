...
# D4: lower intestine only — free permutation (each individual appears once)
PS_D4 <- prune_samples(
...
dir.create("results/D4_lower/plots/29_beta_diversity", recursive = TRUE, showWarnings = FALSE)
dir.create("results/D4_lower/tables/29_beta_diversity", recursive = TRUE, showWarnings = FALSE)
...
dist_list <- list(
  "Bray-Curtis"        = phyloseq::distance(PS_D4, method = "bray"),
  "Jaccard"            = phyloseq::distance(PS_D4, method = "jaccard", binary = TRUE),
  "Weighted UniFrac"   = phyloseq::distance(PS_D4, method = "wunifrac"),
  "Unweighted UniFrac" = phyloseq::distance(PS_D4, method = "unifrac")
)
...
ggsave("results/D4_lower/plots/29_beta_diversity/PCoA_combined.png",
...
make_perm_table <- function(res, dist_name) {
  ...
  flextable(df) %>%
    bold(part = "header") %>%
    set_caption(paste0("PERMANOVA (free permutation, sequential SS) \u2014 ", dist_name, " (D4 lower)")) %>%
...
print(doc, target = "results/D4_lower/tables/29_beta_diversity/beta_diversity_stats.docx")

cat("\nAll beta diversity outputs saved to:\n")
cat("  results/D4_lower/plots/29_beta_diversity/\n")
cat("  results/D4_lower/tables/29_beta_diversity/\n")
