...
dir.create("results/D2_paired/plots/18_alpha_diversity", recursive = TRUE, showWarnings = FALSE)
dir.create("results/D2_paired/tables/18_alpha_diversity", recursive = TRUE, showWarnings = FALSE)

# ============================================================
# Step 1: Compute alpha diversity metrics
# ============================================================

alpha_ps <- estimate_richness(PS_D2, measures = c("Shannon", "Observed"))
alpha_ps$sample <- sample_names(PS_D2)

comm_mat <- if (taxa_are_rows(PS_D2)) {
  t(as.matrix(otu_table(PS_D2)))
} else {
  as.matrix(otu_table(PS_D2))
}
pd_res   <- pd(comm_mat, phy_tree(PS_D2), include.root = TRUE)
...
write.csv(alpha_df, "results/D2_paired/tables/18_alpha_diversity/alpha_raw.csv",
          row.names = FALSE)
...
dodge_w <- 0.7
xpos <- c(dahli_lower = 1 - dodge_w/2,
          dahli_upper = 1 + dodge_w/2,
          port_lower  = 2 - dodge_w/2,
          port_upper  = 2 + dodge_w/2)
...
ggsave("results/D2_paired/plots/18_alpha_diversity/alpha_diversity_combined.png",
...
print(doc, target = "results/D2_paired/tables/18_alpha_diversity/lmm_results.docx")
...
write.csv(desc_tbl, "results/D2_paired/tables/18_alpha_diversity/descriptive_stats.csv",
          row.names = FALSE)

ft_desc <- flextable(desc_tbl) %>%
  bold(part = "header") %>%
  set_caption("Alpha diversity descriptive statistics by Species and Intestine type — D2") %>%
  autofit()

doc2 <- read_docx() %>% body_add_flextable(ft_desc)
print(doc2, target = "results/D2_paired/tables/18_alpha_diversity/descriptive_stats.docx")

cat("\nAll alpha diversity outputs saved to:\n")
cat("  results/D2_paired/plots/18_alpha_diversity/\n")
cat("  results/D2_paired/tables/18_alpha_diversity/\n")
