...
# Subset: individuals with both upper AND lower intestine
MET_all   <- data.frame(as.data.frame(sample_data(PS_merged_sanger)), stringsAsFactors = FALSE)
paired_ids <- names(table(MET_all$individual_ID)[table(MET_all$individual_ID) == 2])
PS_D2     <- prune_samples(MET_all$individual_ID %in% paired_ids, PS_merged_sanger)
PS_D2     <- prune_taxa(taxa_sums(PS_D2) > 0, PS_D2)

dir.create("results/D2_paired/plots/39_intragroup_variance", recursive = TRUE, showWarnings = FALSE)
dir.create("results/D2_paired/tables/39_intragroup_variance", recursive = TRUE, showWarnings = FALSE)
...
bc_dist <- phyloseq::distance(PS_D2, method = "bray")
...
ft <- flextable(coef_df) %>%
  bold(part = "header") %>%
  autofit() %>%
  set_caption("LMM: distance to species centroid (Bray-Curtis) ~ Species + sample_type + (1|individual_ID). D2 — paired 44 samples.")

doc <- read_docx() %>% body_add_flextable(ft)
print(doc, target = "results/D2_paired/tables/39_intragroup_variance/lmm_distance_to_centroid_D2.docx")
write.csv(coef_df, "results/D2_paired/tables/39_intragroup_variance/lmm_distance_to_centroid_D2.csv",
          row.names = FALSE)
...
p_bd <- ggplot(bd_df, aes(x = Species, y = distance, fill = Species)) +
  ...
  labs(
    ...
    title    = "Interindividual variance — D2 (paired samples)",
...
ggsave("results/D2_paired/plots/39_intragroup_variance/distance_to_centroid_D2.png",
...
write.csv(summary_pw,
          "results/D2_paired/tables/39_intragroup_variance/pairwise_within_summary_D2.csv",
          row.names = FALSE)
...
ggsave("results/D2_paired/plots/39_intragroup_variance/pairwise_within_D2.png",
       p_pw, width = 6, height = 5.5, dpi = 300, bg = "white")

cat("\nDone — D2 intragroup variance (", nsamples(PS_D2), "samples)\n")
