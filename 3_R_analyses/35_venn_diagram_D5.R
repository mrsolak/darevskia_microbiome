...
OTU <- as.data.frame(as.matrix(otu_table(PS_D5)))
if (!taxa_are_rows(PS_D5)) OTU <- t(OTU)

dir.create("results/D5_portschinskii/plots/35_venn_diagram", recursive = TRUE, showWarnings = FALSE)
dir.create("results/D5_portschinskii/tables/35_venn_diagram", recursive = TRUE, showWarnings = FALSE)

get_asv_set <- function(lifestage, type, prev_threshold = 0) {
  samps <- rownames(MET)[MET$Life.stage == lifestage & MET$sample_type == type]
  n     <- length(samps)
...
# 4 groups: Juv Upper, Juv Lower, Adult Upper, Adult Lower
group_labels    <- c("Juv Upper", "Juv Lower", "Adult Upper", "Adult Lower")
group_lifestage <- c("Juv",   "Juv",   "Adult", "Adult")
...
cat("=== Sample counts per group (D5) ===\n")
for (i in seq_along(group_labels)) {
  samps <- rownames(MET)[MET$Life.stage == group_lifestage[i] & MET$sample_type == group_types[i]]
  cat(sprintf("  %-22s  n = %d\n", group_labels[i], length(samps)))
}

all_sets <- lapply(thresholds, function(thr) {
  setNames(
    mapply(get_asv_set, group_lifestage, group_types,
...
make_venn <- function(sets, title) {
  ...
  # Juv groups: #56B4E9 (light blue), Adult groups: #D55E00 (vermillion)
  p +
    ...
    scale_color_manual(values = c("#56B4E9", "#56B4E9", "#D55E00", "#D55E00")) +
    labs(title = title) +
    theme(
...
p_combined <- wrap_plots(panels, nrow = 1) +
  plot_annotation(
    title = "ASV overlap by Life stage and intestine type — D5 D. portschinskii",
    theme = theme(plot.title = element_text(face = "bold", size = 13, hjust = 0.5))
  )

ggsave("results/D5_portschinskii/plots/35_venn_diagram/venn_4way_thresholds.png",
       p_combined, width = 18, height = 7, dpi = 300, bg = "white")

p_single <- make_venn(all_sets[[1]], "No threshold (\u2265 1 sample)")
ggsave("results/D5_portschinskii/plots/35_venn_diagram/venn_4way.png",
       p_single, width = 8, height = 7, dpi = 300, bg = "white")

set_names_short <- c("juv_upper", "juv_lower", "adult_upper", "adult_lower")
...
for (k in seq_along(thresholds)) {
  ...
  write.csv(summary_df,
            sprintf("results/D5_portschinskii/tables/35_venn_diagram/venn_summary_thr%s.csv", thr_tag),
            row.names = FALSE)
  write.csv(as.data.frame(overlap_mat),
            sprintf("results/D5_portschinskii/tables/35_venn_diagram/venn_overlaps_thr%s.csv", thr_tag),
            row.names = TRUE)
}

cat("\nOutputs saved to:\n")
cat("  results/D5_portschinskii/plots/35_venn_diagram/\n")
cat("  results/D5_portschinskii/tables/35_venn_diagram/\n")
