...
# D4: lower intestine only — each individual appears once, no random effect needed
PS_D4 <- prune_samples(
...
dir.create("results/D4_lower/plots/28_alpha_diversity", recursive = TRUE, showWarnings = FALSE)
dir.create("results/D4_lower/tables/28_alpha_diversity", recursive = TRUE, showWarnings = FALSE)

# ============================================================
# Step 1: Compute alpha diversity metrics
# ============================================================

alpha_ps <- estimate_richness(PS_D4, measures = c("Shannon", "Observed"))
alpha_ps$sample <- sample_names(PS_D4)

comm_mat <- if (taxa_are_rows(PS_D4)) {
  t(as.matrix(otu_table(PS_D4)))
} else {
  as.matrix(otu_table(PS_D4))
}
pd_res   <- pd(comm_mat, phy_tree(PS_D4), include.root = TRUE)
...
write.csv(alpha_df, "results/D4_lower/tables/28_alpha_diversity/alpha_raw.csv",
          row.names = FALSE)
...
# ============================================================
# Step 2: Linear model + Wilcoxon test
# ============================================================
...
ggsave("results/D4_lower/plots/28_alpha_diversity/alpha_diversity_combined.png",
...
make_lm_table <- function(anova_res, wilcox_p, metric_name) {
  ...
  flextable(df_out) %>%
    bold(part = "header") %>%
    set_caption(paste0("LM / Wilcoxon results \u2014 ", metric_name, " (D4 lower intestine)")) %>%
    autofit()
}
...
print(doc, target = "results/D4_lower/tables/28_alpha_diversity/lm_results.docx")
...
write.csv(desc_tbl, "results/D4_lower/tables/28_alpha_diversity/descriptive_stats.csv",
          row.names = FALSE)

ft_desc <- flextable(desc_tbl) %>%
  bold(part = "header") %>%
  set_caption("Alpha diversity descriptive statistics by Species — D4 lower intestine") %>%
  autofit()

doc2 <- read_docx() %>% body_add_flextable(ft_desc)
print(doc2, target = "results/D4_lower/tables/28_alpha_diversity/descriptive_stats.docx")

cat("\nAll alpha diversity outputs saved to:\n")
cat("  results/D4_lower/plots/28_alpha_diversity/\n")
cat("  results/D4_lower/tables/28_alpha_diversity/\n")
