...
# Harmonise sample_type labels (same recode as 11_first_look)
...
dir.create("results/D1_all/plots/13_alpha_diversity", recursive = TRUE, showWarnings = FALSE)
dir.create("results/D1_all/tables/13_alpha_diversity", recursive = TRUE, showWarnings = FALSE)

# ============================================================
# Step 1: Compute alpha diversity metrics
# ============================================================

# Shannon and Observed from phyloseq
alpha_ps <- estimate_richness(PS_merged_sanger, measures = c("Shannon", "Observed"))
# estimate_richness() applies make.names() to sample names (hyphens → dots), so map
# back to original names using the preserved order from sample_names()
alpha_ps$sample <- sample_names(PS_merged_sanger)

# Faith's PD via picante
# use as.matrix (not as.data.frame) to avoid make.names() mangling ASV sequence column names
comm_mat <- if (taxa_are_rows(PS_merged_sanger)) {
  t(as.matrix(otu_table(PS_merged_sanger)))
} else {
  as.matrix(otu_table(PS_merged_sanger))
}
pd_res   <- pd(comm_mat, phy_tree(PS_merged_sanger), include.root = TRUE)
...
alpha_df <- left_join(alpha_ps,
                      MET[, c("sample", "Species", "sample_type",
                               "Life.stage", "individual_ID")],
                      by = "sample")

write.csv(alpha_df, "results/D1_all/tables/13_alpha_diversity/alpha_raw.csv",
          row.names = FALSE)
...
# ============================================================
# Step 2: Linear mixed models (run before plot to build annotations)
# Species * sample_type fixed; individual_ID random
# ============================================================

metrics <- c("Observed", "Shannon", "FaithPD")

run_lmm <- function(df, metric) {
  fml <- as.formula(paste(metric, "~ Species * sample_type + (1 | individual_ID)"))
...
dodge_w <- 0.7
# Fill groups sorted alphabetically: "Lower intestine" → left, "Upper intestine" → right
xpos <- c(dahli_lower = 1 - dodge_w/2,   # 0.65
          dahli_upper = 1 + dodge_w/2,   # 1.35
          port_lower  = 2 - dodge_w/2,   # 1.65
          port_upper  = 2 + dodge_w/2)   # 2.35

# --- emmeans pairwise contrasts (unadjusted) ---
get_pairwise_p <- function(mod) {
  emm <- emmeans(mod, ~ Species * sample_type)
  ...
  list(
    dahli_LU  = find_p("dahli Lower intestine",         "dahli Upper intestine"),
    port_LU   = find_p("portschinskii Lower intestine",  "portschinskii Upper intestine"),
    lower_DP  = find_p("dahli Lower intestine",         "portschinskii Lower intestine"),
    upper_DP  = find_p("dahli Upper intestine",         "portschinskii Upper intestine")
  )
}
...
# --- Bracket data frame (per metric, 4 comparisons) ---
build_brackets <- function(m, pv) {
  ...
  data.frame(
    ...
    x1    = c(xpos["dahli_lower"], xpos["port_lower"],  xpos["dahli_lower"], xpos["dahli_upper"]),
    x2    = c(xpos["dahli_upper"], xpos["port_upper"],  xpos["port_lower"],  xpos["port_upper"]),
    # between-species brackets offset vertically to avoid overlap
    y_br  = c(y_max + step,        y_max + step,        y_max + 2.8*step,   y_max + 4.0*step),
    tick  = tick,
    label = c(fmt_stars(pv$dahli_LU), fmt_stars(pv$port_LU),
              fmt_stars(pv$lower_DP), fmt_stars(pv$upper_DP)),
...
cat("\n=== Pairwise emmeans contrasts (unadjusted) ===\n")
for (m in metrics) {
  cat("\n---", m, "---\n")
  pv <- pw_pvals[[m]]
  cat("  D.dahli Lower vs Upper:          p =", round(pv$dahli_LU, 4), "\n")
  cat("  D.portschinskii Lower vs Upper:  p =", round(pv$port_LU,  4), "\n")
  cat("  Lower: D.dahli vs D.portschinskii: p =", round(pv$lower_DP, 4), "\n")
  cat("  Upper: D.dahli vs D.portschinskii: p =", round(pv$upper_DP, 4), "\n")
}

# --- Strip labels: metric name + overall LMM stats ---
...
make_strip <- function(metric_label, anova_res) {
  ...
  paste0(
    metric_label, "\n",
    "Species: F=", round(a["Species",       "F value"], 2), " ", fmt_p_compact(a["Species",       "Pr(>F)"]),
    "  |  Sample type: F=", round(a["sample_type",    "F value"], 2), " ", fmt_p_compact(a["sample_type",    "Pr(>F)"]),
    "  |  Interaction: F=", round(a["Species:sample_type", "F value"], 2), " ", fmt_p_compact(a["Species:sample_type", "Pr(>F)"])
  )
}
...
p_combined <- ggplot(alpha_long, aes(x = Species_abbr, y = value, fill = sample_type)) +
...
ggsave("results/D1_all/plots/13_alpha_diversity/alpha_diversity_combined.png",
       p_combined, width = 8, height = 12, dpi = 300, bg = "white")

# ============================================================
# Step 4: Save LMM results as .docx
# (metrics vector already defined above)
# ============================================================
...
make_lmm_table <- function(anova_res, metric_name) {
  ...
  df_out$Term <- gsub("Species:sample_type", "Species \u00d7 Sample type", df_out$Term)
  df_out$Term <- gsub("sample_type", "Sample type", df_out$Term)
  flextable(df_out) %>%
    bold(part = "header") %>%
    set_caption(paste0("LMM results \u2014 ", metric_name)) %>%
    autofit()
}
...
print(doc, target = "results/D1_all/tables/13_alpha_diversity/lmm_results.docx")
...
write.csv(desc_tbl, "results/D1_all/tables/13_alpha_diversity/descriptive_stats.csv",
          row.names = FALSE)

ft_desc <- flextable(desc_tbl) %>%
  bold(part = "header") %>%
  set_caption("Alpha diversity descriptive statistics by Species and Intestine type") %>%
  autofit()

doc2 <- read_docx() %>%
  body_add_flextable(ft_desc)
print(doc2, target = "results/D1_all/tables/13_alpha_diversity/descriptive_stats.docx")

cat("\nAll alpha diversity outputs saved to:\n")
cat("  results/D1_all/plots/13_alpha_diversity/\n")
cat("  results/D1_all/tables/13_alpha_diversity/\n")
