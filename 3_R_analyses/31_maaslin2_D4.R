...
# D4: lower intestine only — Species only model, no random effects
PS_D4 <- prune_samples(
...
dir.create("results/D4_lower/plots/31_maaslin2",  recursive = TRUE, showWarnings = FALSE)
dir.create("results/D4_lower/tables/31_maaslin2", recursive = TRUE, showWarnings = FALSE)
...
for (rank in tax_levels) {
  ...
  otu_mat <- prepare_level(PS_D4, rank)
  ...
  for (thr_name in names(threshold_configs)) {
    ...
    out_dir  <- file.path("results/D4_lower/tables/31_maaslin2", tag)
    plot_dir <- file.path("results/D4_lower/plots/31_maaslin2",  tag)
    ...
    fig_title    <- paste0("Differential abundance \u2014 ", rank, " level (D4 lower)")
...
cat("\n=== All MaAsLin2 D4 analyses complete ===\n")
cat("Results: results/D4_lower/tables/31_maaslin2/\n")
cat("Figures: results/D4_lower/plots/31_maaslin2/\n")
