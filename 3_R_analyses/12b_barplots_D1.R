...
dir.create("results/D1_all/plots/12b_barplots", recursive = TRUE, showWarnings = FALSE)
...
df <- make_phylum_df(PS_merged_sanger)
phyla_ordered <- df %>%
...
p <- ggplot(df, aes(x = sample, y = rel_abund, fill = Phylum)) +
  ...
  labs(x = NULL, y = "Relative abundance", fill = "Phylum",
       title = "Phylum-level composition — D1 all samples (n = 49)")

ggsave("results/D1_all/plots/12b_barplots/barplot_species_sampletype.png",
       p, width = 14, height = 6, dpi = 300, bg = "white")

cat("Barplot saved to results/D1_all/plots/12b_barplots/\n")
