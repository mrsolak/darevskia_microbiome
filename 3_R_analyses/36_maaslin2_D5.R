...
dir.create("results/D5_portschinskii/plots/36_maaslin2",  recursive = TRUE, showWarnings = FALSE)
dir.create("results/D5_portschinskii/tables/36_maaslin2", recursive = TRUE, showWarnings = FALSE)
...
make_heatmap <- function(res, title, subtitle, rank) {
  ...
  df <- res %>%
    ...
    mutate(
      term  = ifelse(metadata == "Life.stage",
                     "Life stage\n(Juv vs Adult)",
                     "Sample type\n(Upper vs Lower)"),
      stars = case_when(
        qval < 0.001 ~ "***",
  ...
  ls_order <- df %>%
    filter(metadata == "Life.stage") %>%
    arrange(coef) %>%
    pull(feature)
  feat_order <- unique(c(ls_order, setdiff(sig_feats, ls_order)))
  ...
  ggplot(df, aes(x = term, y = feature, fill = coef)) +
    ...
    scale_fill_gradient2(
      low = "#56B4E9", mid = "white", high = "#D55E00",
    ...
    labs(
      title    = title,
      subtitle = subtitle,
      caption  = paste0("Stars: *** q<0.001  ** q<0.01  * q<0.05")
    )
}
...
make_volcano <- function(res, title, subtitle) {
  ...
  ggplot(df, aes(x = coef, y = neg_log_q, colour = direction)) +
    ...
    scale_colour_manual(
      values = c("ns" = "grey75", "positive" = "#D55E00", "negative" = "#56B4E9"),
      guide  = "none"
    ) +
    theme_bw(base_size = 11) +
    theme(
    ...
    labs(
      ...
      caption  = paste0("Dashed line: q = ", SIG_Q,
                        "  |  Reference groups: Life stage = Adult, Sample type = Lower intestine",
                        "\n  Life stage panel — orange: enriched in Juveniles; blue: enriched in Adults (ref)",
...
make_scatter <- function(res, title, subtitle) {
  ...
  ls_coef  <- grep("coef_Life.stage",  names(wide), value = TRUE)[1]
  st_coef  <- grep("coef_sample_type", names(wide), value = TRUE)[1]
  ls_qval  <- grep("qval_Life.stage",  names(wide), value = TRUE)[1]
  st_qval  <- grep("qval_sample_type", names(wide), value = TRUE)[1]

  if (anyNA(c(ls_coef, st_coef, ls_qval, st_qval))) {
    return(
      ggplot() +
        annotate("text", x = 0.5, y = 0.5,
  ...
  wide <- wide %>%
    rename(c_ls = !!ls_coef, c_st = !!st_coef,
           q_ls = !!ls_qval, q_st = !!st_qval) %>%
    mutate(
      sig_cat = case_when(
        q_ls < SIG_Q & q_st < SIG_Q ~ "Both",
        q_ls < SIG_Q                 ~ "Life stage only",
  ...
  ggplot(wide, aes(x = c_ls, y = c_st, colour = sig_cat)) +
    ...
    scale_colour_manual(
      values = c(
        "Both"             = "#CC79A7",
        "Life stage only"  = "#D55E00",
        "Sample type only" = "#56B4E9",
        "Neither"          = "grey75"
      ),
    ...
    labs(
      title    = title,
      subtitle = subtitle,
      x        = "Coefficient: Life stage (Juv vs Adult)",
...
for (rank in tax_levels) {
  ...
  otu_mat <- prepare_level(PS_D5, rank)
  ...
  for (thr_name in names(threshold_configs)) {
    ...
    out_dir  <- file.path("results/D5_portschinskii/tables/36_maaslin2", tag)
    plot_dir <- file.path("results/D5_portschinskii/plots/36_maaslin2",  tag)
    ...
    fig_title    <- paste0("Differential abundance \u2014 ", rank, " level (D5)")
...
cat("\n=== All MaAsLin2 D5 analyses complete ===\n")
cat("Results: results/D5_portschinskii/tables/36_maaslin2/\n")
cat("Figures: results/D5_portschinskii/plots/36_maaslin2/\n")
