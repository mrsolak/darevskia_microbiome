...
dir.create("results/D3_upper/plots/22_barplots", recursive = TRUE, showWarnings = FALSE)
...
p <- ggplot(df, aes(x = sample, y = rel_abund, fill = Phylum)) +
  ...
  labs(x = NULL, y = "Relative abundance", fill = "Phylum",
       title = sprintf("Phylum-level composition — D3 upper intestine (n = %d)", nsamples(PS_D3)))

ggsave("results/D3_upper/plots/22_barplots/barplot_species.png",
       p, width = 12, height = 6, dpi = 300, bg = "white")

cat("Barplot saved to results/D3_upper/plots/22_barplots/\n")
