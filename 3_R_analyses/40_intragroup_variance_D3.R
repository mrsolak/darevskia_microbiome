...
# Subset: upper intestine only
PS_D3 <- subset_samples(PS_merged_sanger, sample_type == "Upper intestine")
PS_D3 <- prune_taxa(taxa_sums(PS_D3) > 0, PS_D3)

dir.create("results/D3_upper/plots/40_intragroup_variance", recursive = TRUE, showWarnings = FALSE)
dir.create("results/D3_upper/tables/40_intragroup_variance", recursive = TRUE, showWarnings = FALSE)
...
bc_dist <- phyloseq::distance(PS_D3, method = "bray")
...
cat("\n=== LM: distance to centroid ~ Species (D3 upper intestine) ===\n")
print(mod_sum)
...
ft <- flextable(coef_df) %>%
  bold(part = "header") %>%
  autofit() %>%
  set_caption("LM: distance to species centroid (Bray-Curtis) ~ Species. D3 — upper intestine only.")

doc <- read_docx() %>% body_add_flextable(ft)
print(doc, target = "results/D3_upper/tables/40_intragroup_variance/lm_distance_to_centroid_D3.docx")
write.csv(coef_df, "results/D3_upper/tables/40_intragroup_variance/lm_distance_to_centroid_D3.csv",
          row.names = FALSE)
...
write.csv(summary_bd,
          "results/D3_upper/tables/40_intragroup_variance/betadisper_summary_D3.csv",
          row.names = FALSE)
...
p_bd <- ggplot(bd_df, aes(x = Species, y = distance, fill = Species)) +
  ...
  labs(
    ...
    title    = "Interindividual variance — D3 (upper intestine only)",
...
ggsave("results/D3_upper/plots/40_intragroup_variance/distance_to_centroid_D3.png",
...
write.csv(summary_pw,
          "results/D3_upper/tables/40_intragroup_variance/pairwise_within_summary_D3.csv",
          row.names = FALSE)
...
ggsave("results/D3_upper/plots/40_intragroup_variance/pairwise_within_D3.png",
       p_pw, width = 5.5, height = 5, dpi = 300, bg = "white")

cat("\nDone — D3 intragroup variance (", nsamples(PS_D3), "samples)\n")
